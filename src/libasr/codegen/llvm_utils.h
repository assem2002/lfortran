...
class ASRToLLVMVisitor;
    ...
    /* ===== Utilities ===== */
        ...
        void insert_BB_finalize_variable_name(){
            LCOMPILERS_ASSERT_MSG(!variable_symbol_stack.empty(),
                                    "You're requesting a type without a variable symbol in stack; "
                                    "If it's of structType we won't be able to deduce its type")
            insert_BB((std::string("Finalize_Variable_")+std::string(variable_symbol_stack.top()->m_name)).c_str());
        }
        ...
        void finlaize_type_shell(llvm::Value* const var_ptr, ASR::ttype_t* const t){
    ...
    /* === Entry ===*/
        void finalize_symtab(SymbolTable* symtab){
            ...
            // LCOMPILERS_ASSERT(insert_BB("FINALIZE_SYMTABLE")) // For llvm IR readability.
