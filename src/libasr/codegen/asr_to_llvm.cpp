...
public:
    ...
    */
    ...
    void serialize_args_types(ASR::expr_t** args, size_t n_args, std::string &res /*Holds return for efficiency*/){
        for (size_t i=0; i<n_args; i++) {
            create_serialized_type(ASRUtils::expr_type(args[i]), res);
    ...
    void visit_StringFormat(const ASR::StringFormat_t& x) {
        ...
        if (x.m_kind == ASR::string_format_kindType::FormatFortran) {
            ...
            serialize_args_types(x.m_args, x.n_args, serialization_info);
            std::cout<< serialization_info<<std::endl;
