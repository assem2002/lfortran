...
public:
    ...
    void cast_string_to_array(const ASR::Cast_t* const cast){
        ...
        switch(array_ty->m_physical_type){
            ...
            case ASR::DescriptorArray:{
                ...
                if(!array_ty->m_dims->m_start){ // dimension(*) -- Set size -> 1
                    LCOMPILERS_ASSERT(array_ty->n_dims == 1)
                    Vec<ASR::dimension_t> arr_dims; arr_dims.reserve(al, 1);
                    ASRUtils::ASRBuilder b(al, cast->base.base.loc);
                    arr_dims.push_back(al, {cast->base.base.loc, b.i32(1), b.i32(1)});
    ...
    */
    ...
    void check_strings_phsyicalType_match(ASR::ttype_t* const a, ASR::ttype_t* const b){
        ...
        || ASRUtils::type_get_past_allocatable(b)->type == ASR::Array) return; //Workaround for now
